...
//		// TODO Auto-generated method stub
		...
		String[] xepLoai = xepLoaiHocLuc(dTB);
		...
		do {
			...
			case '2':
				...
				inDSSV(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case '3':
				xepLoai = xepLoaiHocLuc(dTB);
				inDSSV(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case '4':
				inDSSV(hoten, dToan, dVan, dAnh, dTB, xepLoai);
			...
			case '5':
				...
				break;
			case '6':
				sapxepDSSV(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				inDSSV(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case '7':
				timkiemDiem(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case '8':
				timHoTen(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case '9':
				timSVCoDTBCaoNhat(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case 'x':
				xuatFile(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case 'z':
				nhapFile(hoten, dToan, dVan, dAnh, dTB, xepLoai);
				break;
			case'Q':
...
//Ham tao menu
	public static void inMenu() {
		...
		System.out.println("(3): XẾP LOẠI HỌC LỰC SINH VIÊN");
		System.out.println("(4): IN DANH SÁCH SINH VIÊN");
		System.out.println("(5): SỬA DỮ LIỆU SINH VIÊN");
		System.out.println("(6): SẮP XẾP SINH VIÊN THEO ĐIỂM TRUNG BÌNH");
		System.out.println("(7): TÌM KIẾM THEO ĐIỂM TOÁN");
		System.out.println("(8): TÌM SINH VIÊN THEO HỌ VÀ TÊN");
		System.out.println("(9): TÌM SINH VIÊN CÓ ĐIỂM TRUNG BÌNH CAO NHẤT");
		System.out.println("(x): XUẤT FILE VĂN BẢN");
		System.out.println("(z): NHẬP FILE VĂN BẢN");	
		System.out.println("*******************************************************");
	}
...
// Ham tinh diem trung binh
	...
	public static String[] xepLoaiHocLuc(double[] dTB) {
		String[] xepLoai = new String[dTB.length];
		for (int i = 0; i < dTB.length; i++) {
			if (dTB[i] >= 9.0) {
				xepLoai[i] = "Xuất sắc";
			} else if (dTB[i] >= 8.0) {
...
// Ham sap xep sinh vien theo chieu giam dan cua diem trung binh
	public static void sapxepDSSV(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB,String[] xepLoai) {
...
// Ham in danh sach sinh vien ra man hinh
	public static void inDSSV(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB,
		...
		System.out.println("+---------------------+-----------+-----------+-----------+-----------+----------------+");
		System.out.printf("| %-20s| %-10s| %-10s| %-10s| %-10s| %-15s|%n", "Họ và tên", "Toán", "Văn", "Anh", "Điểm TB",
				"Xếp loại");
		System.out.println("+---------------------+-----------+-----------+-----------+-----------+----------------+");

		for (int i = 0; i < hoten.length; i++) {
			System.out.printf("| %-20s| %-10.2f| %-10.2f| %-10.2f| %-10.2f| %-15s|%n", hoten[i], dToan[i], dVan[i],
					dAnh[i], dTB[i], xepLoai[i]);
		}
				...
				for (int i = 0; i < dToan.length; i++) {
					if (dToan[i] == diemcantim) {
						if (count == 0) {
							...
							System.out.println(
									"+---------------------+-----------+-----------+-----------+-----------+----------------+");
							System.out.printf("| %-20s| %-10s| %-10s| %-10s| %-10s| %-15s|%n", "Họ và tên", "Toán",
									"Văn", "Anh", "Điểm TB", "Xếp loại");
							System.out.println(
									"+---------------------+-----------+-----------+-----------+-----------+----------------+");
						}
						System.out.printf("| %-20s| %-10.2f| %-10.2f| %-10.2f| %-10.2f| %-15s|%n", hoten[i], dToan[i], dVan[i],
...
// Ham tim kiem sinh vien theo ho ten
	public static void timHoTen(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB,String[] xepLoai) {
		int count = 0;
		Scanner sc = new Scanner(System.in);
		String x;
		try {
	...
	public static void timSVCoDTBCaoNhat(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB, String[] xepLoai) {
	    double max = dTB[0];
	    ...
	    System.out.println("+---------------------+-----------+-----------+-----------+-----------+----------------+");
	    System.out.printf("| %-20s| %-10s| %-10s| %-10s| %-10s| %-15s|%n", "Họ và tên", "Toán", "Văn", "Anh", "Điểm TB", "Xếp loại");
	    System.out.println("+---------------------+-----------+-----------+-----------+-----------+----------------+");
	    for (int i = 0; i < dTB.length; i++) {
	        if (dTB[i] == max) {
	            System.out.printf("| %-20s| %-10.2f| %-10.2f| %-10.2f| %-10.2f| %-15s|%n", hoten[i], dToan[i], dVan[i], dAnh[i], dTB[i], xepLoai[i]);
	        }
	    }
	}
	

	public static void xuatFile(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB, String[] xepLoai) {
	    try {
	        FileWriter fw = new FileWriter("danhsachsinhvien.txt");
	        for (int i = 0; i < hoten.length; i++) {
	            fw.write(hoten[i] + " - Điểm Toán: " + dToan[i] + " - Điểm Văn: " + dVan[i] + " - Điểm Anh: " + dAnh[i]
	                    + " - Điểm trung bình: " + dTB[i] +" - Xếp loại: " + xepLoai[i] + "\n");
	        }
	        fw.close();
	...
	public static void nhapFile(String[] hoten, double[] dToan, double[] dVan, double[] dAnh, double[] dTB,String[] xepLoai) {
		try {
